dashboard "issue_my_open_age_report" {
  title = "GitLab My Open Issues Age Report"
  documentation = file("./dashboards/issue/docs/issue_my_report_age.md")
  ...
  container {
    card {
      query = query.my_open_issue_count
      width = 2
    }

    card {
      query = query.my_open_issue_24_hours_count
      width = 2
      type  = "info"
    }

    card {
      query = query.my_open_issue_30_days_count
      width = 2
      type  = "info"
    }

    card {
      query = query.my_open_issue_30_90_days_count
      width = 2
      type  = "info"
    }

    card {
      query = query.my_open_issue_90_365_days_count
      width = 2
      type  = "info"
    }

    card {
      query = query.my_open_issue_1_year_count
      width = 2
      type  = "info"
    }
  }
  ...
  container {
    table {
      ...
      query = query.my_open_issue_table
...
query "my_open_issue_24_hours_count" {
  sql = <<-EOQ
    select
      '< 24 Hours' as label,
      count(i.*) as value
...
query "my_open_issue_30_days_count" {
  sql = <<-EOQ
    select
      '1-30 Days' as label,
      count(i.*) as value
...
query "my_open_issue_90_365_days_count" {
  sql = <<-EOQ
    select
